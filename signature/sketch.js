/* ============================================================
   Signature System — sketch.js (슬라이더 단일 생성기)
   ------------------------------------------------------------
   errorA/errorB 슬라이더로 하나의 그래픽을 생성해서 보여준다.
   실제 그래픽 생성 로직(형태·색상 규칙)은 core.js 참고 —
   archive-sketch.js와 이 로직을 공유한다.
...
const PADDING_RATIO = 0.1; // 캔버스 가장자리와 사각형이 유지할 여백 = 캔버스 크기 × 이 비율
...
// #canvas-wrap이 실제로 차지하는 공간(뷰포트 크기에서 컨트롤 영역을 뺀 나머지) 중
// 작은 쪽에 맞춰 정사각형 캔버스 크기를 계산한다.
...
// ── 저장 ────────────────────────────────────────────────────
function saveImg() {
  saveCanvas(`signature_A${eA}_B${eB}`, 'png');
}
