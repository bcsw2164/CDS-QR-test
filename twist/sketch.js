/* ============================================================
   Twisted Bezier Flower — sketch.js (슬라이더 단일 생성기)
   ------------------------------------------------------------
   errorA/errorB 슬라이더로 베지어 뒤틀림 꽃잎 그래픽 하나를 생성해서
   보여준다. 실제 그래픽 생성 로직(형태 규칙)은 core.js의
   drawTwistedBezierFlower 참고.
...
const PADDING_RATIO = 0.1; // 캔버스 가장자리와 꽃이 유지할 여백 = 캔버스 크기 × 이 비율
...
function draw() {
  ...
  drawTwistedBezierFlower(window, cx, cy, size, eA, eB);
}

// ── 저장 ────────────────────────────────────────────────────
function saveImg() {
  saveCanvas(`twist_A${eA.toFixed(3)}_B${eB.toFixed(3)}`, 'png');
}
